/*==================================================
 vmcore.js 1.5.1 built in 2016.1.2
 ==================================================*/
(function(global, factory) {
    ...
    var vmcoreFragment = DOC.createDocumentFragment()
    ...
    vmcore.isFunction = typeof alert === "object" ? function(fn) {
        try {
    ...
    vmcore.mix = vmcore.fn.mix = function() {
        ...
        //确保接受方为一个复杂的数据类型
        if (typeof target !== "object" && !vmcore.isFunction(target)) {
            target = {}
        }
    ...
    var directives = vmcore.directives = {}
    vmcore.directive = function(name, obj) {
    ...
    vmcore.contains = function(root, el) {
    ...
    function $watch(expr, binding) {
        ...
        if (!binding.update) {
            if (/\w\.*\B/.test(expr) || expr === "*") {
                ...
                queue.sync = true
                vmcore.Array.ensure(queue, binding)
            } else {
                vmcore.injectBinding(binding)
            }
            if (backup) {
    ...
    function observe(obj, old, hasReturn, watch) {
        ...
        } else if (vmcore.isPlainObject(obj)) {
    ...
    vmcore.injectBinding = function(binding) {
        ...
        binding.update = function() {
            ...
            try {
                ...
                log("warning:exception throwed in [vmcore.injectBinding] ", e)
    ...
    function injectDependency(list, binding) {
        if (binding.oneTime)
            return
        if (list && vmcore.Array.ensure(list, binding) && binding.element) {
    ...
    var disposeQueue = vmcore.$$subscribers = []
    ...
    function injectDisposeQueue(data, list) {
        ...
        vmcore.Array.ensure(lists, list)
            ...
            while (data = disposeQueue[--i]) {
                ...
                if (iffishTypes[data.type] && shouldDispose(data.element)) { //如果它没有在DOM树
                    ...
                    for (var k = 0, list; list = lists[k++];) {
                        vmcore.Array.remove(lists, list)
                        vmcore.Array.remove(list, data)
                    }
                    disposeData(data)
                }
            }
    ...
    function shouldDispose(el) {
        ...
        return el.msRetain ? 0 : (el.nodeType === 1 ? !root.contains(el) : !vmcore.contains(root, el))
    }
    ...
    vmcore.parseHTML = function(html) {
        var fragment = vmcoreFragment.cloneNode(false)
    ...
    vmcore.fireDom = function(elem, type, opts) {
        if (DOC.createEvent) {
            var hackEvent = DOC.createEvent("Events");
            hackEvent.initEvent(type, true, true, opts)
            vmcore.mix(hackEvent, opts)
    ...
    vmcore.cssNumber = oneObject("animationIterationCount,columnCount,order,flex,flexGrow,flexShrink,fillOpacity,fontWeight,lineHeight,opacity,orphans,widows,zIndex,zoom")
    ...
    function normalizeExpr(code) {
        var hasExpr = rexpr.test(code) //比如vm-class="width{{w}}"的情况
        if (hasExpr) {
    ...
    vmcore.normalizeExpr = normalizeExpr
    vmcore.parseExprProxy = parseExpr
    ...
    vmcore.scan = function(elem, vmodel) {
        elem = elem || root
    ...
    var rnoCollect = /^(vm-\S+|data-\S+|on[a-z]+|id|style|class)$/
    var ronattr = /^on\-[\w-]+$/
    ...
    function scanAttr(elem, vmodels, match) {
        ...
        if (vmodels.length) {
            ...
            for (var i = 0, attr; attr = attributes[i++];) {
                var name = attr.name
                if (uniq[name]) { //IE8下vm-repeat,vm-with BUG
                    continue
                }
                uniq[name] = 1
                if (attr.specified) {
                    if (match = name.match(rmsAttr)) {
                        ...
                        if (directives[type]) {
                            ...
                            if (type === "html" || type === "text") {
                                ...
                                var hasDuplex = name
                            } else if (name === "vm-if-loop") {
                                binding.priority += 100
                            } else if (name === "vm-attr-value") {
            ...
            if (bindings.length) {
                ...
                if (hasDuplex && hasAttrValue && elem.nodeName === "INPUT" && elem.type === "text") {
                    log("warning!一个控件不能同时定义vm-attr-value与" + hasDuplex)
                }
    ...
    function scanNodeList(parent, vmodels) {
        var nodes = vmcore.slice(parent.childNodes)
                ...
                case 1:
                    ...
                    if (!elem.msResolved && elem.parentNode && elem.parentNode.nodeType === 1) {
                        ...
                        if (library) {
                            ...
                            if (vmcore.components[fullName]) {
                                //确保所有vm-attr-name扫描完再处理
                    ...
                    if (node.msHasEvent) {
                        vmcore.fireDom(node, "datasetchanged", {
    ...
    function scanTag(elem, vmodels, node) {
        ...
        if (newVmodel) {
            setTimeout(function() {
                newVmodel.$fire("vm-scan-end", elem)
            })
        }
    }
    ...
    function scanText(textNode, vmodels, index) {
        ...
        if (tokens.length) {
            for (var i = 0, token; token = tokens[i++];) {
                ...
                vmcoreFragment.appendChild(node)
            }
            textNode.parentNode.replaceChild(vmcoreFragment, textNode)
            if (bindings.length)
                executeBindings(bindings, vmodels)
        }
    }
    ...
    var componentHooks = {
        $construct: function() {
            return vmcore.mix.apply(null, arguments)
        },
        $ready: noop,
        $init: noop,
        $dispose: noop,
    ...
    vmcore.components = {}
    vmcore.component = function(name, opts) {
        if (opts) {
            vmcore.components[name] = vmcore.mix({}, componentHooks, opts)
        }
        for (var i = 0, obj; obj = componentQueue[i]; i++) {
            if (name === obj.fullName) {
                ...
                (function(host, hooks, elem, widget) {
                    ...
                    //issuse : https://github.com/RubyLouvre/vmcore2/issues/40
                    if (!vmcore.contains(DOC, elem) || elem.msResolved) {
                        vmcore.Array.remove(componentQueue, host)
                        return
                    }

                    var dependencies = 1
                    var library = host.library
                    var global = vmcore.libraries[library] || componentHooks

                    //===========收集各种配置=======
                    if (elem.getAttribute("vm-attr-identifier")) {
                        //如果还没有解析完,就延迟一下 #1155
                        return
                    }
                    ...
                    var parentHooks = vmcore.components[hooks.$extend]
                    if (parentHooks) {
                        vmcore.mix(true, componentDefinition, parentHooks)
                        componentDefinition = parentHooks.$construct.call(elem, componentDefinition, {}, {})
                    } else {
                        vmcore.mix(true, componentDefinition, hooks)
                    }
                    componentDefinition = vmcore.components[name].$construct.call(elem, componentDefinition, vmOpts, elemOpts)
                    ...
                    var vmodel = vmcore.define(componentDefinition) || {}
                    ...
                    for (s in slots) {
                        if (vmodel.hasOwnProperty(s)) {
                            var ss = slots[s]
                            if (ss.length) {
                                var fragment = vmcoreFragment.cloneNode(true)
                    ...
                    if (keepReplace) {
                        ...
                        if (className) {
                            vmcore(elem).addClass(className)
                        }
                    }
                    if (keepContainer) {
                        keepContainer.appendChild(elem)
                    }
                    vmcore.fireDom(elem, "datasetchanged", {
                        library: library,
                        vm: vmodel,
                        childReady: 1
                    })
                    var children = 0
                    var removeFn = vmcore.bind(elem, "datasetchanged", function(e) {
                        ...
                        if (dependencies === 0) {
                            ...
                            vmcore.unbind(elem, "datasetchanged", removeFn)
                                //==================
                    ...
                    vmcore.vmodels[vmodel.$id] = vmodel
                ...
                })(obj, vmcore.components[name], obj.element, obj.widget) // jshint ignore:line
    ...
    vmcore.libraries = []
    vmcore.library = function(name, opts) {
        if (DOC.namespaces) {
            DOC.namespaces.add(name, 'http://www.w3.org/1999/xhtml');
        }
        vmcore.libraries[name] = vmcore.mix({
            $init: noop,
            $ready: noop,
            $dispose: noop
        }, opts || {})
    ...
    //各种MVVM框架在大型表格下的性能测试
    // https://github.com/RubyLouvre/vmcore/issues/859
    ...
    var attrDir = vmcore.directive("attr", {
        init: function(binding) {
            ...
            if (binding.type === "include") {
                ...
                var outer = binding.includeReplace = !!vmcore(elem).data("includeReplace")
                if (vmcore(elem).data("includeCache")) {
                    binding.templateCache = {}
                }
                binding.start = DOC.createComment("vm-include")
                binding.end = DOC.createComment("vm-include-end")
                if (outer) {
    ...
    vmcore.directive("class", {
        init: function(binding) {
            ...
            if (!oldStyle || isFinite(oldStyle)) {
                ...
                log('vm-' + method + '-xxx="yyy"这种用法已经过时,请使用vm-' + method + '="xxx:yyy"')
    ...
    vmcore.directive("css", {
        init: directives.attr.init,
        update: function(val) {
            vmcore(this.element).css(this.param, val)
        }
    })

    vmcore.directive("data", {
        priority: 100,
        update: function(val) {
            var elem = this.element
    ...
    var duplexBinding = vmcore.directive("duplex", {
        priority: 2000,
        init: function(binding, hasCast) {
            ...
            switch (binding.xtype) {
                ...
                case "select":
                    bound("change", function() {
                        var val = vmcore(elem).val() //字符串或字符串数组
            ...
            if (binding.xtype === "input" && !rnoduplexInput.test(elem.type)) {
                ...
                elem.vmcoreSetter = updateVModel //#765
                watchValueInTimer(function() {
                    if (vmcore.contains(root, elem)) {
                        if (!this.msFocus) {
                            updateVModel()
                        }
        ...
        update: function(value) {
            ...
            if (!this.init) {
                for (var i in vmcore.vmodels) {
                    var v = vmcore.vmodels[i]
                    v.$fire("vmcore-vm-duplex-init", binding)
                }
    ...
    vmcore.directive("effect", {
        ...
        init: function(binding) {
            ...
            if (colonIndex === -1) { // 比如 vm-class/effect="aaa bbb ccc" 的情况
                className = text
                rightExpr = true
            } else { // 比如 vm-class/effect-1="ui-state-active:checked" 的情况
    ...
    vmcore.effects = {}
    vmcore.effect = function(name, callbacks) {
        vmcore.effects[name] = callbacks
    }
    ...
    var transitionDuration = vmcore.cssName("transition-duration")
    var animationDuration = vmcore.cssName("animation-duration")
    new function() { // jshint ignore:line
        var checker = {
    ...
    function effectFactory(el, opts) {
        ...
        if (instance.useCss) {
            opts && vmcore(el).addClass(opts.effectClass)
            instance.cssEvent = driver === "t" ? transitionEndEvent : animationEndEvent
        }
        instance.name = name
        instance.callbacks = vmcore.effects[name] || {}
    ...
    Effect.prototype = {
        ...
        actionFun: function(name, before, after) {
            ...
            var cssCallback = function(cancel) {
                ...
                if (isLeave) {
                    ...
                    vmcore(el).removeClass(me.cssClass)
                } else {
                    if (me.driver === "a") {
                        vmcore(el).removeClass(me.cssClass)
                    }
            ...
            if (me.useCss) {
                ...
                vmcore(el).addClass(me.cssClass) //animation会立即触发
        ...
        enter: function(before, after) {
            this.actionFun.apply(this, ["enter"].concat(vmcore.slice(arguments)))

        },
        leave: function(before, after) {
            this.actionFun.apply(this, ["leave"].concat(vmcore.slice(arguments)))
    ...
    vmcore.directive("html", {
        update: function(val) {
            ...
            if (typeof val !== "object") { //string, number, boolean
                var fragment = vmcore.parseHTML(String(val))
            } else if (val.nodeType === 11) { //将val转换为文档碎片
                fragment = val
            } else if (val.nodeType === 1 || val.item) {
                var nodes = val.nodeType === 1 ? val.childNodes : val.item
                fragment = vmcoreFragment.cloneNode(true)
                while (nodes[0]) {
            ...
            if (isHtmlFilter) {
                ...
                vmcore.clearHTML(elem).appendChild(fragment)
            }
        ...
        update: function(val) {
            ...
            if (val) { //插回DOM树
                ...
                if (elem.nodeType === 8) {
                    ...
                    var hasEffect = vmcore.effect.apply(keep, 1, function() {
                        if (stamp !== binding.stamp)
                            return
                        ...
                        if (elem.querySelectorAll) {
                            vmcore.each(elem.querySelectorAll("[_required=true]"), function(el) {
                                el.required = true
                ...
                if (elem.nodeType === 1) {
                    ...
                    try { //如果不支持querySelectorAll或:required,可以直接无视
                        vmcore.each(elem.querySelectorAll(":required"), function(el) {
                            elem.required = false
                            el.setAttribute("_required", "true")
                        })
                    } catch (e) {}

                    var node = binding.element = DOC.createComment("vm-if"),
                    ...
                    vmcore.effect.apply(elem, 0, function() {
                        binding.recoverNode = null
                        if (stamp !== binding.stamp)
                            return
    ...
    var templatePool = vmcore.templateCache = {}
    ...
    vmcore.directive("include", {
        ...
        update: function(val) {
            ...
            binding.recoverNodes = binding.recoverNodes || vmcore.noop
            var scanTemplate = function(text) {
                ...
                // 元素退场
                vmcore.effect.remove(leaveEl, target, function() {
                ...
                vmcore.effect.apply(enterEl, "enter", before, after)
            }
    ...
    var rdash = /\(([^)]*)\)/
    var onDir = vmcore.directive("on", {
        priority: 3000,
        init: function(binding) {
            var value = binding.expr
            binding.type = "on"
            var eventType = binding.param.replace(/-\d+$/, "") // vm-on-mousemove-10
    ...
    vmcore.directive("repeat", {
        ...
        init: function(binding) {
            ...
            if (elem.nodeType === 1) {
                ...
                binding.start = start
                binding.template = vmcoreFragment.cloneNode(false)
                if (type === "repeat") {
                ...
                if (rendered) {
                    var removeFn = vmcore.bind(parent, "datasetchanged", function() {
                        rendered.apply(parent, parent.args)
                        vmcore.unbind(parent, "datasetchanged", removeFn)
        ...
        update: function(value, oldValue) {
            ...
            var fragments = []
            var transation = init && vmcoreFragment.cloneNode(false)
            var proxies = []
            var param = this.param
            var retain = vmcore.mix({}, this.cache)
            var elem = this.element
            ...
            var id = setTimeout(function() {
                ...
                vmcore.fireDom(parent, "datasetchanged", {
    ...
    "with,each".replace(rword, function(name) {
        directives[name] = vmcore.mix({}, directives.repeat, {
            priority: 1400
        })
    })
    ...
    function animateRepeat(nodes, isEnter, binding) {
        for (var i = 0, node; node = nodes[i++];) {
            if (node.className === binding.effectClass) {
                vmcore.effect.apply(node, isEnter, noop, noop, binding)
            }
        }
    }
    ...
    function removeItem(node, binding) {
        ...
        var nodes = vmcore.slice(last.parentNode.childNodes)
            ...
            if (binding && (pre.className === binding.effectClass)) {
                ...
                (function(cur) {
                    binding.staggerIndex = mayStaggerAnimate(binding.effectLeaveStagger, function() {
                        vmcore.effect.apply(cur, 0, noop, function() {
        ...
        update: function(val) {
            ...
            if (!this.stamp) {
                ...
                if (val) {
                    ...
                    if (vmcore(elem).css("display") === "none") {
            ...
            stamp = this.stamp = +new Date()
            if (val) {
                vmcore.effect.apply(elem, 1, function() {
                    if (stamp !== binding.stamp)
                        return
    ...
    vmcore.bind(window, "load", fireReady)
